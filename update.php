...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($allHeaders['Content-Type'] == 'application/json') {
		if ($allHeaders['Authorization']==$securityKey) {
			if(!empty($data->id)){
				$sql = "SELECT `id` FROM `users` WHERE `id` = '".$data->id."'";
				...
				if ($result <= 0){
					...
					if(!empty($data->user_name) && !empty($data->user_pass) && !empty($data->user_email) && !empty($data->user_group_id) && !empty($data->previlege)){
						$update_sql = "UPDATE `users` SET `user_name`='".$data->user_name."',`user_pass`='".$data->user_pass."',`user_email`='".$data->user_email."',`user_group_id`='".$data->user_group_id."',`previlege` = '".$data->previlege."' WHERE `id`='".$data->id."'";
						$update = $crud->update($update_sql);
						if($update > 0){
							echo json_encode(
