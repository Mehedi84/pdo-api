	...
	if ($allHeaders['Content-Type'] == 'application/json') {
		if ($allHeaders['Authorization']==$securityKey) {
			if(!empty($data->user_name) && !empty($data->user_pass) && !empty($data->user_email) && !empty($data->user_group_id) && !empty($data->previlege)){
				...
				$create_sql ="INSERT INTO `users`(`user_name`, `user_pass`, `user_email`, `user_group_id`, `previlege`) VALUES ('".$data->user_name."','".$data->user_pass."','".$data->user_email."','".$data->user_group_id."','".$data->previlege."')";
				$create = $crud->create($create_sql);
				if($create > 0){
					echo json_encode(
